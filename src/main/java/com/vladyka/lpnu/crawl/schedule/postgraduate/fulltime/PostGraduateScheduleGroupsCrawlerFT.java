...
@Service
public class PostGraduateScheduleGroupsCrawlerFT implements ScheduleCrawler {
    ...
    @Override
    public void crawl() {
        logger.info("[Postgraduate schedule, Full-time] - Started crawling groups dropDown");
        List<String> groups = parseService.parseGroups(
                parseUrlProvider.getPostGraduateBaseUrlFT());
        groupService.createAll(groups, POST_GRADUATE_GROUP, FULL_TIME);
        logger.info("[Postgraduate schedule, Full-time] - Finished crawling groups dropDown, total {} groups", groups.size());
    }
}
